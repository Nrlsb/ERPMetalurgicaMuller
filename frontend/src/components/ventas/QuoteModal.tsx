...
import { FileCheck2, Plus, Trash2, X, Calendar } from 'lucide-react';
...
export function QuoteModal({ customers, products, onSuccess, onClose }: QuoteModalProps) {
  ...
  const removeItem = (index: number) => {
    if (form.items.length === 1) {
      warning('Debe incluir al menos un producto en la cotización.');
      return;
    }
    setForm({
      ...form,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.customerId) {
      error('Seleccione un cliente para la cotización');
      return;
    }

    setIsSubmitting(true);
    try {
      const res = await fetchApi('/sales/quotes', {
        method: 'POST',
        body: JSON.stringify(form),
      });

      if (res.success) {
        success(`Presupuesto ${res.data?.code || ''} generado exitosamente`);
        onSuccess(res.data);
      } else {
        error(res.message || 'Error al generar la cotización');
      }
    } catch (e: any) {
      error('Error de red al guardar la cotización');
    } finally {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-4xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/90">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Generar Cotización / Presupuesto</h2>
              <p className="text-xs text-slate-400">Emite propuestas comerciales con validez temporal</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-800/40 p-4 rounded-2xl border border-slate-700/50">
            <div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">Productos / Ítems Cotizados</h3>
              <button
                type="button"
                onClick={addItem}
                ...
                >
                  ...
                  <div className="col-span-2">
                    <input
                      ...
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-center font-bold text-white"
                      placeholder="Cant."
                    />
                  </div>
                  ...
                  <div className="col-span-2">
                    <input
                      ...
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-right text-white"
                      placeholder="Precio"
                    />
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-800">
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1">Notas de la Cotización</label>
              <textarea
                value={form.notes}
                onChange={(e) => setForm({ ...form, notes: e.target.value })}
                placeholder="Condiciones comerciales, validez, plazos de entrega..."
                rows={3}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
            ...
            <button
              ...
              className="px-6 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-sm font-bold text-white shadow-lg shadow-purple-500/20 transition-all"
            >
